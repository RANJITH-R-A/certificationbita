...
export default function CertificationPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen relative">
      ...
      <button
        className="md:hidden p-3 bg-blue-600 text-white absolute top-4 left-4 rounded-md z-50"
        onClick={() => setIsSidebarOpen(!isSidebarOpen)}
      >
        {isSidebarOpen ? "Close" : "Open Certification Category" }
      </button>

      {/* Sidebar */}
      <aside
        className={` absolute md:relative z-10 top-0 left-0 h-full bg-gray-50 shadow-md border-r border-gray-200 p-6 transition-transform duration-300 ${
