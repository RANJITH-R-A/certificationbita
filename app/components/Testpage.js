...
// import certifications from "../data/CertificationName.json";
  ...
  return (
    <div className=" relative flex flex-col md:flex-row h-screen lg:h-auto">
      ...
      {/* Main Content */}
              ...
              <button
        className="flex gap-2 justify-center md:hidden p-2 mx-6 my-4 bg-blue-600 text-white rounded-md w-80"
        onClick={() => setIsSidebarOpen(true)}
      >
        <Menu size={24} />Open All Certification Categories
      </button>
              ...
              (subcategory) =>
                (!selectedCategory || selectedCategory === subcategory.name) && (
                  <div key={subcategory.name} className="mb-6">
                    <h2 className="text-lg font-bold text-blue-600 my-5">{subcategory.name}</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-2">
                      {subcategory.items.map((item) => (
                        <div key={item.code} className="border p-1 rounded-lg flex items-center space-x-3 my-3 shadow-md bg-white">
